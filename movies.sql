...
from  movies.actor  join movies.movie_cast
on actor.act_id =movie_cast. act_id
 join movies.movie
 on movies.movie_cast.mov_id=movie.mov_id
WHERE mov_year not  BETWEEN 1990 AND 2000;

4)
select  mov_year
from movies.movie  join  movies.rating
...
from movies.movie  join  movies.movie_direction
on movie.mov_id =movie_direction.mov_id
join  movies.director
on movies.director.dir_id= movie_direction.dir_id
join movies.rating
...
from movies. movie
join movies. rating ON rating.mov_id =movie.mov_id where rating.rev_stars=(SELECT MAX(rating.rev_stars) FROM movies.rating);
